...
RSpec.describe Interaction, type: :model do
  ...
  scenario "one_of_three_interactions returns error if sentence is blank" do
    ...
    expect(interaction.one_of_three_interactions).to eq([
      "You need to fill the sentence field",
      "You need to fill the sentence field"
    ])
  end

  scenario "one_of_three_interactions returns error
    if keyword1 is blank choosing category keyword" do
    interaction = Interaction.create(
      category: "Keyword",
      sentiment: "Positive",
      keyword1: "",
    ...
    expect(interaction.one_of_three_interactions).to eq([
      "You need to fill the keyword field",
      "You need to fill the keyword field"
    ])
  end

  scenario "one_of_three_interactions returns error
    if keyword1 is blank choosing category combo" do
    interaction = Interaction.create(
      category: "Combo",
      sentiment: "Positive",
      keyword1: "",
  ...
  scenario "one_of_three_interactions returns error
    if keyword2 is blank choosing category combo" do
    interaction = Interaction.create(
      category: "Combo",
      sentiment: "Positive",
  ...
  scenario "one_of_three_interactions returns error if keyword1 and keyword2
    are blank choosing category combo" do
    interaction = Interaction.create(
      category: "Combo",
      sentiment: "Positive",
      keyword1: "",
